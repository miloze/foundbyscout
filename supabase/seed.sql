...
-- Migrates all hardcoded data into Supabase

-- ── PARKS ──────────────────────────────────────────────────────────────────

INSERT INTO parks (
  slug, name, address, postcode, borough, location, type, surface, surface_note,
  is_free, is_covered, opened, builder, managed_by, description,
  brief, facts, lat, lng, hero_image, model_file, use_contour_model,
  camera_pos, model_rotation,
...
(
  ...
  ARRAY['8.5ft cloverleaf pool', 'L-shaped bowl 5.5–7ft', 'Street section', 'Crystal Palace Park'],
  51.4156, -0.0719,
  '/images/parks/crystal-palace/gallery-01.webp',
  '/crystal-palace-skate_park.glb', true,
  ARRAY[-18.0, 20.0, 20.0]::numeric[], ARRAY[-1.5707963, 0.0, 0.0]::numeric[],
  '[
  ...
  '[
    {"name":"Cloverleaf Pool","description":"8.5ft deep, tile and coping. Three connected kidney shapes — the deepest and most technical section of the park.","position":"North end","bounty":"First proper frontside grind on the pool coping","difficulty":"hard"},
    {"name":"L-Shaped Bowl","description":"Transitions from 5.5ft to 7ft across its length. Flows naturally into the pool section.","position":"Centre","bounty":"Best line linking all three bowl sections without stopping","difficulty":"medium"},
    {"name":"Street Section","description":"Mellow banks, a ledge, and flat ground. The most accessible part of the park — a good warm-up before the bowls.","position":"South end","bounty":"Best nosegrind on the main ledge","difficulty":"easy"}
  ]'::jsonb,
  '[
...
(
  ...
  ARRAY[0.0, 7.0, 18.0]::numeric[], ARRAY[0.0, 3.14159265, 0.0]::numeric[],
  '[
  ...
  '[]'::jsonb,
  '[
    {"name":"The Bump","description":"The centrepiece. A low concrete wedge that''s been the launch pad for fifty years of clips. Deceptively technical — the low ceiling changes everything.","position":"Centre","bounty":"Best trick over the bump — anything that clears the pillar","difficulty":"medium"},
    {"name":"The Banks","description":"Smooth angled concrete running the length of the undercroft. Every style of skating works here — the columns create natural lines.","position":"Both sides","bounty":"Longest line using both banks without touching flat","difficulty":"easy"},
    {"name":"The Ledges","description":"Original concrete ledges ground down by decades of wax and grinding. Each one slightly different — nothing is perfectly square.","position":"Throughout","bounty":"Switch nosegrind the main ledge","difficulty":"hard"},
    {"name":"Manual Pad","description":"Flat-top concrete block at the south end. Low enough to be approachable, worn enough to be honest.","position":"South end","bounty":"Nose manual the full length","difficulty":"medium"}
  ]'::jsonb,
  '[
...
(
  ...
  ARRAY[0.0, 5.0, 31.0]::numeric[], ARRAY[0.0, 1.5707963, 0.0]::numeric[],
  '[
    {"icon":"wb_sunny","value":"Outdoor","label":"Setting","available":true},
    {"icon":"bolt","value":"No floodlights","label":"Lighting","available":false},
...
(
  'why-skateparks-look-the-way-they-do',
  'Why Skateparks Look the Way They Do',
  'Essay',
  'Sixty years of concrete, community, and compromise.',
  true, 4
),
(
  'scotlands-hidden-parks',
  'Scotland''s Hidden Parks',
  'Regional',
  'From Livingston to the islands — the parks that don''t make the lists.',
  true, 5
);
